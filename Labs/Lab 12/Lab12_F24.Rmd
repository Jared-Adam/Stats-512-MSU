...
```{r}
nm1 <- lmer(GripStrength ~ forearm_bins + GripTesterID + Hand +  (1|SubjectID/GroupName), data = combinedR)
Anova(nm1)
summary(nm1)
plot(allEffects(nm1))
```
...
```{r}
nm2 <- lmer(GripStrength ~ forearm_bins + Hand +  (1|SubjectID/GroupName), data = combinedR)
...
All of our group estimated strengths are higher than the average, after accounting for subject and group. 
