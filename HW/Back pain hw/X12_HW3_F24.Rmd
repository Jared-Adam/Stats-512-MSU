...
ggplot(g3, aes(y = sittime, x = as.factor(likert_scale)))+
  geom_violin()+
  geom_point()+
  facet_wrap(~occlift)+
...
```
**It is hard to tease out a trend in the potential interaction based on this figure. It looks like the spread of observations is greater in the rarely lifting category than the other two. However, for all three lifting categories, it appears that the likert scale-2 value has the most observations while 0 has the fewest. The high group appears to have no trend, the moderate group appears to have a positive relationship between sit time and likert scale, and the rarely group appears to have a negative relationship.**
...
* $\hat{\mu}\{sittime|influence\ast occlift\} = 6.9878 + 0.1386     (likertscale) - 1.7615 \cdot I_{Occlift=moderate} + 0.5800 \cdot I_{Occlift=rarely} + 0.8663(likertscale)\cdot I_{Occlift = moderate} - 0.2904(likertscale)\cdot I_{Occlift = rarely})$
...
$\hat{\mu}\{sittime|influence\ast (occlift = high)\} = 6.9878 + 0.1386(likertscale) - 1.7615 \cdot 0 + 0.5800 \cdot 0 + 0.8663(likertscale)\cdot 0 - 0.2904(likertscale)\cdot 0)$
 
$\hat{\mu}\{sittime|influence\ast (occlift = high)\} = 6.9878 + 0.1386(likertscale)$
 
**For moderate:**
 
$\hat{\mu}\{sittime|influence\ast (occlift = moderate)\} = 6.9878 + 0.1386(likertscale) - 1.7615 \cdot 1 + 0.5800 \cdot 0 + 0.8663(likertscale)\cdot 1 - 0.2904(likertscale)\cdot 0)$
 
$\hat{\mu}\{sittime|influence\ast (occlift = moderate)\} = 5.2263 + 1.0049(likertscale)$
 
**For rarely:**
 
$\hat{\mu}\{sittime|influence\ast (occlift = rarely)\} = 6.9878 + 0.1386(likertscale) - 1.7615 \cdot 0 + 0.5800 \cdot 1 + 0.8663(likertscale)\cdot 0 - 0.2904(likertscale)\cdot 1)$
 
$\hat{\mu}\{sittime|influence\ast (occlift = rarely)\} = 7.5678 - 0.1518(likertscale)$
